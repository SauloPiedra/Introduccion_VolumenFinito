...
ALLOCATE(x(0:nx),xc(0:nx+1),y(0:ny),yc(0:ny+1))
...
WRITE(3,*)'unset key; set xrange[0:10]; set yrange[0:1]; set size ratio 0.1'
!**************************************************************************************
...
!     u1(ei+1,:)=u1(ei,:)	!Cálculo de u en frontera Neumann
...
!     v1(ei+1,:)=v1(ei,:)	!Cálculo de V en forntera Neumann
    ...
    u1(nx,:)=u1(nx-1,:)	!Correción de velocidad U
    ...
    v1(nx+1,:)=v1(nx,:)	!Correción de velocidad V
  ...
  IF (MOD(it,1000) .EQ. 0) THEN
    ...
    WRITE(3,*)"p 'vel"//itchar(1:LEN_TRIM(itchar))//".txt' u 1:2:(0.35*$3):(0.35*$4)w vec"
!     WRITE(3,*)'pause 0.05'
  END IF
...
implicit none
integer nx,ny
real*4:: us(0:nx,0:ny+1),uc(0:nx+1,0:ny+1)
...
implicit none
integer nx,ny
real*4:: vs(0:nx+1,0:ny),vc(0:nx+1,0:ny+1)
integer bi,ei,bj,ej,i,j
bi=1; bj=1;ej=ny; ei=nx
